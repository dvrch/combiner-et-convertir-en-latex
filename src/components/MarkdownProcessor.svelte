<script lang="ts">
    ...
    export let processedContent: string = '';
    ...
    export async function processMarkdown(inputContent: string): Promise<string> {
        processedFiles.clear();
        return await processEmbeddedLinks(inputContent);
    }

    export async function getUniqueFileName(baseName: string, parentPath: string): Promise<string> {
        let counter = 1;
